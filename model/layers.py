...
from resnet_backbone import identity_block as bottleneck
...
def create_global_net(blocks, cfg, has_bn=True, bn_trainable=True):
    ...
    for i, block in enumerate(reversed(blocks)):
        ...
        out = KL.Conv2D(cfg.NR_SKELETON, (3, 3), padding="SAME", \
        ...
        out = KL.Lambda(lambda t: tf.image.resize(t, \
...
def create_refine_net(blocks, cfg, use_bn=False):
    refine_fms = []
    for i, block in enumerate(blocks):
        ...
        mid_fm = KL.Lambda(lambda t: tf.image.resize(t, \
                (cfg.OUTPUT_SHAPE[0], cfg.OUTPUT_SHAPE[1])),\
    ...
    res = KL.Conv2D(cfg.NR_SKELETON, (3, 3),
